...
- **NASA 사진**: 태양, 행성, 달, 안드로메다은하의 관측 자료 11장을 한 화면에서 탐색합니다. 각 카드의 관찰 질문에 답하고 `3D에서 보기`로 모형과 비교할 수 있습니다. 관측 파장, 합성 여부와 NASA 원본 링크를 표시합니다.
- 천체 정보의 **빛의 여행 시간**은 태양과 행성 사이 평균 거리(장반경)에 1 AU ≈ 8.317 광분을 곱한 학습용 근삿값입니다. 실제 시간은 공전 위치에 따라 달라집니다.

사진은 `public/images/nasa/`에 포함되어 인터넷 연결 없이도 표시됩니다. 각 사진의 원본과 제공 기관은 `src/data/media.ts` 및 사진 카드의 `NASA 원본` 링크에서 확인할 수 있습니다. 태양계 시뮬레이션의 거리·크기는 학습용 모드에서 과장되어 있으며, 사진은 시뮬레이션 날짜에 촬영된 영상이 아닙니다.
