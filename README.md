...
- **사진 아틀라스**: NASA 관측 자료 18장과 생성한 학습용 개념 일러스트 2장을 탐색합니다. 사진을 눌러 크게 열고 `←` `→` 키나 화면 버튼으로 넘겨 보며, `Esc`로 닫습니다. 각 카드의 관찰 질문에 답하고 `3D에서 보기`로 모형과 비교할 수 있습니다. 관측 파장, 합성 여부, 원본 링크와 일러스트 구분을 표시합니다.
...
관측 자료는 `public/images/nasa/`, 생성 일러스트는 `public/images/illustrations/`에 포함되어 인터넷 연결 없이도 표시됩니다. 관측 사진의 원본과 제공 기관은 `src/data/media.ts` 및 사진 카드의 `NASA 원본` 링크에서 확인할 수 있습니다. 일러스트는 OpenAI ImageGen으로 만든 낮과 밤, 암석 행성과 가스 행성의 개념 그림으로 실제 관측 사진이나 크기·거리의 정확한 도표가 아닙니다. 태양계 시뮬레이션의 거리·크기는 학습용 모드에서 과장되어 있으며, 사진은 시뮬레이션 날짜에 촬영된 영상이 아닙니다.
